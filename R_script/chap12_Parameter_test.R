...
z.test(x, alternative = 'two.sided', mu=168,  # x는 표본
...
set.seed(10)
x <- rnorm(29, mean=168, sd = 3) # N(168, 3^2) 
hist(x) # 좌우 대칭 확인  
mean(x) # 표본평균
sd(x) # 표본표준편차 
...
W =shapiro.test(x) # 정규성검정 : 정규분포와 차이가 없다.
W # W = 0.97309, p-value = 0.6459 > 0.05 
